...
export const AssesmentTemplatePage: React.FC = (): any => {
  ...
  const metaData = {
    title: "Assesment Template -  Sales App Ekatunggal",
    description: "Halaman Template sales web system",
  };
      ...
      if (result.data.length > 0) {
        const generateData = result.data.map((item: any): IDataTables => {
          return {
            ...
            doc: item.name,
            name: (
              <Link to={`/assesment/template/${item._id}`}>
  ...
  useKey("n", () => alert("Create new Template"), {
    ctrl: true,
    alt: true,
  });
      ...
      onConfirm: async (): Promise<void> => {
        ...
        try {
          ...
          for (const item of data) {
            await GetDataServer(DataAPI.ASSESMENTTEMPLATE).DELETE(item.id);
        ...
        {!loading ? (
          <>
            <div className=" w-full h-16 flex items-center justify-between">
              ...
              <div className="flex-1  flex items-center justify-end mr-4">
                <IconButton
                  ...
                  callback={() => navigate("/assesment/template/new")}
                />
            ...
            <TableComponent
              ...
              filter={filter}
              setFilter={setFilter}
              localStorage={LocalStorageType.FILTERASSESMENTTEMPLATE}
              onRefresh={onRefresh}
            />
          </>
        ) : (
