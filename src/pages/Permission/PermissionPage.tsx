...
export const PermissionPage: React.FC = (): any => {
  ...
  const metaData = {
    ...
    description: "Halaman Permission - sales web system",
  };
  ...
  const getData = async (): Promise<any> => {
    try {
      const result: any = await GetDataServer(DataAPI.PERMISSION).FIND({
        limit: limit,
        page: page,
        filters: filter,
  ...
  useKey("n", () => alert("Create new Permission"), {
    ctrl: true,
    alt: true,
  });
      ...
      onConfirm: async (): Promise<void> => {
        ...
        try {
          ...
          for (const item of data) {
            await GetDataServer(DataAPI.PERMISSION).DELETE(item.id);
        ...
        {!loading ? (
          <>
            ...
            <TableComponent
              ...
              filter={filter}
              setFilter={setFilter}
              localStorage={LocalStorageType.FILTERPERMISSION}
              onRefresh={onRefresh}
            />
          </>
        ) : (
