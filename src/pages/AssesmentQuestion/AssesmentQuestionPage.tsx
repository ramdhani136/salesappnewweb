...
export const AssesmentQuestionPage: React.FC = (): any => {
  ...
  const metaData = {
    title: "Assesment Question -  Sales App Ekatunggal",
    description: "Halaman assesment question sales web system",
  };
      ...
      if (result.data.length > 0) {
        const generateData = result.data.map((item: any): IDataTables => {
          return {
            ...
            doc: item.name,
            name: (
              <Link to={`/assesment/question/${item._id}`}>
  ...
  useKey("n", () => alert("Create new question"), {
    ctrl: true,
    alt: true,
  });
      ...
      onConfirm: async (): Promise<void> => {
        ...
        try {
          ...
          for (const item of data) {
            await GetDataServer(DataAPI.ASSESMENTQUESTION).DELETE(item.id);
        ...
        {!loading ? (
          <>
            <div className=" w-full h-16 flex items-center justify-between">
              ...
              <div className="flex-1  flex items-center justify-end mr-4">
                <IconButton
                  ...
                  callback={() => navigate("/assesment/question/new")}
                />
            ...
            <TableComponent
              ...
              filter={filter}
              setFilter={setFilter}
              localStorage={LocalStorageType.FILTERASSESMENTQUESTION}
              onRefresh={onRefresh}
            />
          </>
        ) : (
