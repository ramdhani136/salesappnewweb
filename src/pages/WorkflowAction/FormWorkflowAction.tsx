...
const FormWorkflowAction: React.FC<any> = ({ props }) => {
  ...
  const metaData = {
    title: `${id ? data.name : "New Action"} - Sales App Ekatunggal`,
    description: "Halaman Form Action - Sales web system",
  };
  ...
  const getData = async (): Promise<void> => {
    try {
      const result = await GetDataServer(DataAPI.WORKFLOWACTION).FINDONE(
        `${id}`
      );
  ...
  const onDelete = (): void => {
    if (id) {
      const progress = async (): Promise<void> => {
        setLoading(true);
        try {
          await GetDataServer(DataAPI.WORKFLOWACTION).DELETE(`${id}`);
          navigate("/workflowaction");
        } catch (error: any) {
          setLoading(false);
          Swal.fire(
          ...
          <>
            ...
            >
              <div className="flex  items-center">
                <h4
                  onClick={() => navigate("/workflowaction")}
...
export default FormWorkflowAction;
