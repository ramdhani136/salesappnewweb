...
export const NamingSeriesPage: React.FC = (): any => {
  ...
  const metaData = {
    title: "Naming Series -  Sales App Ekatunggal",
    description: "Halaman naming series - sales web system",
  };
      ...
      if (result.data.length > 0) {
        const generateData = result.data.map((item: any): IDataTables => {
          return {
            id: item._id,
            name: (
              <Link to={`/namingseries/${item._id}`}>
                <b className="font-medium">{item.name}</b>
              </Link>
            ),
            doc: <div>{item.doc}</div>,
  ...
  useKey("n", () => alert("Create new naming"), {
    ctrl: true,
    alt: true,
  });
      ...
      onConfirm: async (): Promise<void> => {
        ...
        try {
          ...
          for (const item of data) {
            await GetDataServer(DataAPI.NAMING).DELETE(item.id);
        ...
        {!loading ? (
          <>
            <div className=" w-full h-16 flex items-center justify-between">
              ...
              <div className="flex-1  flex items-center justify-end mr-4">
                <IconButton
                  ...
                  callback={() => navigate("/namingseries/new")}
                />
            ...
            <TableComponent
              ...
              filter={filter}
              setFilter={setFilter}
              localStorage={LocalStorageType.FILTERNAMING}
              onRefresh={onRefresh}
            />
          </>
        ) : (
