...
const FormAssesmentPage: React.FC = () => {
  ...
  const metaData = {
    title: `${
      id ? data.name ?? "Loading ..  " : "New Result"
    } - Sales App Ekatunggal`,
    description: "Halaman form result Sales web system",
  };
  ...
  const onDelete = (): void => {
    if (id) {
      const progress = async (): Promise<void> => {
        setLoading(true);
        try {
          await GetDataServer(DataAPI.ASSESMENTRESULT).DELETE(`${id}`);
          navigate("/report/assesment");
        } catch (error: any) {
          setLoading(false);
          Swal.fire(
  ...
  const onSave = async (nextState?: String): Promise<any> => {
    ...
    try {
      ...
      let Action = id
        ? GetDataServer(DataAPI.ASSESMENTRESULT).UPDATE({
            id: id,
            data: data,
          })
        : GetDataServer(DataAPI.ASSESMENTRESULT).CREATE(data);
      ...
      if (id) {
        ...
        navigate(`/assesment/${result.data.data._id}`);
        navigate(0);
      }
    } catch (error: any) {
      Swal.fire(
